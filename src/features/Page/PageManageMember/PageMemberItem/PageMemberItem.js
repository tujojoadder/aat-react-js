...
import { useAddPageAdminMutation, useKickOutPageMemberMutation } from "../../../../services/pagesApi";
import { handleApiError } from "../../../handleApiError/handleApiError";

export default function PageMemberItem({ name, isCreator, image, identifier, isAdmin, isAuth, pageId, newMemberId, isAuthIsCreator }) {
  ...
  const [addAdmin, { isLoading: isAddAdminLoading }] = useAddPageAdminMutation();
  const [kickOutMember, { isLoading: isKickOutLoading }] = useKickOutPageMemberMutation();

  const isLoading = isAddAdminLoading || isKickOutLoading; // Common loading state

  const handleAddAdmin = async () => {
    try {
      await addAdmin({ pageId, newMember: newMemberId }).unwrap();
  ...
  const handleKickOut = async () => {
    try {
      await kickOutMember({ pageId, memberId: newMemberId }).unwrap();
        ...
        <div className="dropdown ms-auto">
          ...
          <ul className="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton">
            ...
            <li>
              ...
              >
                {isLoading ? 'Kicking out...' : 'Kick from page'}
              </a>
            </li>
          </ul>
        </div>
