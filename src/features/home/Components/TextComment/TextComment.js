...
export default function TextComment({ comment }) {
  ...
  useEffect(() => {
    if (comment?.isLove) {
      dispatch(setCommentsLoveReaction({ commentID: comment?.comment_id, isActive: true }));
    }
    if (comment?.isUnlike) {
      dispatch(setCommentsUnlikeReactions({ commentID: comment?.comment_id, isActive: true }));
    }
  }, [dispatch, comment]);
  ...
  const handleUnlikeClick = async () => {
    if (commentUnlikeReactions) {
      dispatch(setCommentsUnlikeReactions({ commentID: comment?.comment_id, isActive: false }));
    } else {
      dispatch(setCommentsUnlikeReactions({ commentID: comment?.comment_id, isActive: true }));
    }
  ...
  const handleReplySubmit = async () => {
    if (replyText.trim()) {
      try {
        const res= await createReply({ commentId: comment?.comment_id, reply_text: replyText });

if (res?.data?.message) {
   // Optionally add the new reply to the local state
   setReplyComments((prev) => [...prev, { id: res?.id, text: replyText }]);
  ...
  return (
    <div className="posts">
      ...
      <div className="user-content-text-box">
        ...
        <div className="content-icons-text py-2 pe-3 d-flex align-items-center" style={{ padding: "0px" }}>
          <i
            className={`far fa-heart ${commentLoveReactions ? "fas red-heart" : ""}`}
            onClick={handleLoveClick}
          >
            {comment?.totalLove > 0 && <span className="ps-1">{comment?.totalLove}</span>}
          </i>
          <i
            className={`far fa-thumbs-down ${commentUnlikeReactions ? "fas black-unlike" : ""}`}
            onClick={handleUnlikeClick}
          >
            {comment?.totalUnlike > 0 && <span className="ps-1">{comment?.totalUnlike}</span>}
          </i>

          <span className="ms-3" style={{ cursor: "pointer", color: "rgb(101, 119, 134)" }} onClick={handleReplyClick}>
            Reply
          </span>
        </div>
          ...
          <div className="reply-input-box me-md-3 mb-3 mt-1 me-sm-1 d-flex align-items-center">
            ...
            <button
              ...
              style={{
                borderRadius: '20px',
                padding: '7px 13px',
                backgroundColor: isSubmitting || !replyText.trim() ? '#ccc' : '#007bff',
                border: 'none',
                color: '#fff',
        ...
        <div className="view-replay-section">
            ...
            >
              View all replies ({replyComments.length})
            </button>
