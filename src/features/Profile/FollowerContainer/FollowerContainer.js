...
export default function FollowerContainer({ userId }) {
  ...
  const {
    data: useGetSpecificUserFollowerQueryData,
    isFetching: useGetSpecificUserFollowerQueryIsFetching,
    isError: useGetSpecificUserFollowerQueryIsError,
    isSuccess: useGetSpecificUserFollowerQueryIsSuccess,
  } = useGetSpecificUserFollowerQuery({ followerPage, userId });

  useEffect(() => {
    if (useGetSpecificUserFollowerQueryIsSuccess) {
      console.log(useGetSpecificUserFollowerQueryData.data);
    }
  }, [useGetSpecificUserFollowerQueryIsSuccess]);
  // Effect to process fetched data
  useEffect(() => {
    if (
      useGetSpecificUserFollowerQueryIsSuccess &&
      useGetSpecificUserFollowerQueryData?.data
    ) {
      if (useGetSpecificUserFollowerQueryData?.data.length === 0) {
        ...
        const newFollowers = useGetSpecificUserFollowerQueryData?.data.filter(
          (newPost) =>
            !allFollower.some(
              (follower) => follower.user_id === newPost.user_id
            )
        );
    ...
    useGetSpecificUserFollowerQueryData,
    useGetSpecificUserFollowerQueryIsSuccess,
  ]);
  ...
  useEffect(() => {
    if (
      fllowerInView &&
      !useGetSpecificUserFollowerQueryIsFetching &&
      !useGetSpecificUserFollowerQueryIsError &&
      hasMoreFollower &&
      useGetSpecificUserFollowerQueryIsSuccess
    ) {
          ...
          {allFollower.map((follower, index) => (
            <div className="col-12 mb-2" key={index}>
              <ShowUserFlower
                ...
                name={`${follower.user_fname} ${follower.user_lname}`} // Combine first and last name
                image={follower.profile_picture}
                handle={follower.identifier}
              />
            </div>
          ))}
        </div>
