...
import { useJoinIChannelMutation, useLeaveIChannelMutation } from "../../../services/iaccountsApi";

export default function IChannelButton({ iChannelId, joinStatus }) {
  const [joinPublicGroup, { isLoading: isJoining }] =
  useJoinIChannelMutation();
  const [leaveGroup, { isLoading: isLeaving }] = useLeaveIChannelMutation();
  ...
  const handleJoinGroup = async () => {
    try {
      await joinPublicGroup(iChannelId).unwrap();
      setIsJoined(true);
    } catch (err) {
  ...
  const handleLeaveGroup = async () => {
    try {
      await leaveGroup(iChannelId).unwrap();
      setIsJoined(false);
    } catch (err) {
      handleApiError(err, dispatch);
    }
        ...
        {!isJoined ? (
          ...
          >
            ...
            <span className={`ms-1 ${isJoining ? "pulsating-text" : ""}`}>
              {isJoining ? "Follow Channe..." : "Follow Channel"}
            </span>
          </div>
        ) : (
          <div
